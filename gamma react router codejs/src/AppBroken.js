...
export default class App extends Component {
    ...
    deleteOrder = () => {
        ...
        axios.delete(`http://localhost:4000/orders/${deletebyId}`).then((res) => {
                // console.log("res:", res.data)
    ...
    deleteUser = () => {
        const deletedUser = this.state.dEmail
        axios.delete(`http://localhost:4000/users/${deletedUser}`).then((res) => {
            console.log("res", res.data);
            // this.setState({ users: res.data })
            this.getUsers()
        }).catch((err) => {
    ...
    render() {
            ...
            return ( <
                    ...
                    to = "/order" > OrdersList < /Link></li >
                    <
                    li > < Link className = "links"
                    to = "/customer" > CustomersList < /Link></li >
                    <
                    li > < Link className = "links"
                    ...
                    Route exact path = "/login" > {
                        this.state.login ? ( < Redirect to = "/order" > < /Redirect>) : (<div className= > <
                            ...
                            render = {
                                ...
                                render = {
                                    ...
                                    <
                                    Route path = "/order" > {
                                        this.state.login ? ( < div > < NewOrder update = { updateOrder }
                                            add = { addOrder }
                                            /> <
                                            ...
                                            input onChange = { orderId }
                                            placeholder = "add Id" / >
                                            <
                                            OrdersList exact orders = { this.state.orders }
                                            /> <
                                            /div>): ( < Link className = "linksLog"
                                                to = "/login" > login first to see the orders < /Link>)}
                                                ...
                                                <
                                                input onChange = { eUserName }
                                                placeholder = "edit UserName" / >
                                                <
                                                input onChange = { eId }
                                                placeholder = "edit Id" / >
                                                <
                                                input onChange = { ePassword }
                                                placeholder = "edit Password" / >
                                                <
                                                input onChange = { ePermission }
                                                placeholder = "edit Permission" / >
                                                <
                                                input onChange = { eBirthDay }
